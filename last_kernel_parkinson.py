...
def load_data():
    ...
    f = open("DNN saves/data_saved._parkinson.json","rt+")
    a = pkl.load(f)
    f.close()
    return a
...
ax.set_xlabel('Epoch')
ax.set_title('Training Cost Parkinson dataset')
...
ax.set_xlabel('Epoch')
ax.set_title('Test Error Parkinson dataset')
fig.savefig(name_string)
...
def save_data():
    import pickle as pkl
    f= open("DNN saves/data_saved_parkinson.json","wt+")
